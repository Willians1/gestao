...
export default function ApiStatusBadge({ compact = false, intervalMs = 30000 }) {
  ...
  const labelBase = () => {
    ...
    switch (h.status) {
      case 'ok':
        return `API OK${suffix}`;
      case 'degraded':
        return `API Lenta${suffix}`;
      case 'offline':
        return `API Offline${suffix}`;
      default:
        return `Verificando…${suffix}`;
    }
  };
  ...
  const details = (
    <div>
      ...
      {h.latencyMs != null && (
        <div>
          <strong>Latência:</strong> {h.latencyMs} ms
        </div>
      )}
      <div>
      ...
      {h.version && (
        <div>
          <strong>Versão:</strong> {h.version}
        </div>
      )}
      {h.commit_sha && (
        <div>
          <strong>Commit:</strong>{' '}
          <span style={{ fontFamily: 'monospace' }}>{String(h.commit_sha).slice(0, 7)}</span>
        </div>
      )}
      {h.build_time && (
        <div>
          <strong>Build:</strong> {h.build_time}
        </div>
      )}
      {typeof h.uptime_seconds === 'number' && (
        <div>
          <strong>Uptime:</strong> {Math.floor(h.uptime_seconds / 3600)}h{' '}
          {Math.floor((h.uptime_seconds % 3600) / 60)}m
        </div>
      )}
      {h.lastCheck && (
        <div>
  ...
  return (
    <Tooltip title={details} arrow disableInteractive>
      <Stack
        ...
        alignItems="center"
        spacing={1}
        sx={{ cursor: 'default', flexWrap: 'wrap' }}
      >
        {isIconOnly ? (
          <Box
            sx={{
        ...
        {!isCompact && (
          <Box sx={{ opacity: 0.8 }}>
            ...
            {h.message && (
              <div>
                <small style={{ color: '#b91c1c' }}>{h.message}</small>
              </div>
            )}
          </Box>
        )}
