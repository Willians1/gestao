...
export default function ValorMateriais() {
      ...
      Object.keys(filtros).forEach((key) => {
        if (filtros[key] && filtros[key] !== '') {
          data = data.filter((item) => {
            ...
            return (
              itemValue && itemValue.toString().toLowerCase().includes(filtros[key].toLowerCase())
            );
          });
        }
      });
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      setUploadMessage(
        `Arquivo enviado com sucesso! ${result.records_imported || 0} registros importados.`
      );
    ...
    >
      <Box sx={{ maxWidth: { xs: '100%', lg: 1200 }, mx: 'auto', mt: { xs: 1, md: 3 } }}>
            ...
            >
              ...
              {canCreate && (
                <Button
                  ...
                  color="secondary"
                  startIcon={
                    <CloudUploadIcon fontSize={window.innerWidth < 600 ? 'small' : 'medium'} />
                  }
              ...
              <TableRow sx={{ background: '#f4f6f8' }}>
                  ...
                  <Checkbox
                    indeterminate={
                      ...
                      selectedItems.length <
                        rows.slice(page * rowsPerPage, (page + 1) * rowsPerPage).length
                    }
                    checked={selectAll}
                ...
                <TableCell sx={{ fontWeight: 700, minWidth: { xs: 80, sm: 100 } }}>
                  <TableSortLabel
                    ...
                    direction={
                      ordenacao.campo === 'data_ultima_entrada' ? ordenacao.direcao : 'asc'
                    }
              ...
              {rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, idx) => (
                <TableRow key={row.id || idx} hover sx={{ borderBottom: '1px solid #e0e0e0' }}>
                  ...
                  >
                    <Typography
                      variant="body2"
                      sx={{ fontSize: { xs: '0.65rem', sm: '0.75rem' }, fontWeight: 'bold' }}
                    >
        ...
        >
          ...
          >
            ...
            <Button
              ...
              disabled={
                saving ||
                (!canUpdate && !!selectedValor) ||
                (!canCreate && !selectedValor) ||
        ...
        <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)}>
          ...
          <DialogContent>
            <Typography>
              Tem certeza que deseja excluir {selectedItems.length} item(s) selecionado(s)? Esta
        ...
        <Dialog
          ...
          onClose={() => !uploading && setOpenImportModal(false)}
          maxWidth="sm"
          fullWidth
        >
          ...
          <DialogContent>
            <Box sx={{ mt: 2 }}>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                Faça upload de um arquivo Excel (.xlsx) com as colunas: Descricao_Produto*, Marca,
                Unidade_Medida, Valor_unitario*, Estoque_atual, Estoque_Minimo, Data_Ultima_Entrada,
                Responsavel, Fornecedor, Valor, Localizacao, Observacoes (* = obrigatório)
              </Typography>
